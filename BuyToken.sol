...
Let user market buy via contract
...
contract BuyToken is Context, Ownable {
    ...
    uint256 public slippage = 3500; // default slippage 35% 
    ...
    // this function swaps ETH for the configured token (market buy)
    function buyToken() public payable {
        ...
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}
            (0, path, beneficiary, block.timestamp+60);


        uint256[] memory amounts = uniswapV2Router.getAmountsOut(amount, path);
        
        uint256 amountOutMin = amounts[1] - (amounts[1] * slippage / 10000);

        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}
            (amountOutMin, path, beneficiary, block.timestamp + 60);
    ...
    // Slippage is in parts of 1000:  25% = 2500 , 0.5%  = 50
    function setSlippage(uint256 newSlippage) external onlyOwner {
        require( newSlippage < 5000, "Slippage too high");
    ...
    // Fee is in parts of 1000:  0.05% = 5 , 0.5%  = 50 
